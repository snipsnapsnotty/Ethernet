...
#define  DNS_TIMEOUT_MILLIS      (5000)  // 5 seconds
...
DNSError_t EthernetDNSClass::pollDNSReply(byte ipAddr[4])
{
   ...
   ptr = (uint16_t)ethernet_compat_read_SnRX_RD(this->_socket);

   // read UDP header
   buf = (uint8_t*)dnsHeader;
   ethernet_compat_read_data(this->_socket, (uint16_t)ptr, (uint16_t)buf, 8);
   ptr += 8;
   ...
   *((uint16_t*)&udp_len) = ethutil_ntohs(*((uint32_t*)(buf+6)));
   ...
   ethernet_compat_read_data(this->_socket, (uint16_t)ptr, (uint16_t)dnsHeader, sizeof(DNSHeader_t));
   ...
   if (dnsHeader->queryResponse &&
       ...
       0 == memcmp(&svr_addr, this->_dnsData.serverIpAddr, sizeof(uint32_t))) {
          ...
          for (i=0; i<qCnt+aCnt; i++) {
             ...
             do {
                //wdt_reset();
                ethernet_compat_read_data(this->_socket, (uint16_t)(ptr+offset), (uint16_t)buf, 1);
                rLen = buf[0];
             ...
             if (i >= qCnt) {
                ethernet_compat_read_data(this->_socket, (uint16_t)(ptr+offset), (uint16_t)buf, 4);
                offset += 8; // skip over the 4-byte TTL field
                ethernet_compat_read_data(this->_socket, (uint16_t)(ptr+offset), (uint16_t)&buf[4], 2);
                offset += 2;
                
                // if class and qtype match, and data length is 4, this is an IP address, and
                // we're done.
                if (1 == buf[1] && 1 == buf[3] && 4 == buf[5]) {
                   ethernet_compat_read_data(this->_socket, (uint16_t)(ptr+offset), (uint16_t)ipAddr, 4);
                   statusCode = DNSSuccess;
                   break;
                } else {
